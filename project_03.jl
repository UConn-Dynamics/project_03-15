...
begin
	# Given project constants
	L = 0.4          # m, bar length
	k = 10.0         # N/m, spring constant
	m1 = 0.1         # kg, block mass
	m2 = 0.3         # kg, bar mass
	g = 9.81         # m/s^2
	
	# Moment of inertia of slender bar about its center
	J1 = 1e-6        # block rotation inertia, tiny because theta1 is constrained
	J2 = (1/12)*m2*L^2
	
	nq = 6           # number of generalized coordinates
...
# ╔═╡ 3ff04b62-4a91-4c91-aaa2-76cf371e35ae
function Cq_matrix(q)
	x1, y1, θ1, x2, y2, θ2 = q
	...
	# C3 = x2 - L/2 cos(theta2) - x1
	Cq[3,1] = -1
	Cq[3,4] = 1
	Cq[3,6] = (L/2)*sin(θ2)
	
	# C4 = y2 - L/2 sin(theta2) - y1
	Cq[4,2] = -1
	Cq[4,5] = 1
	Cq[4,6] = -(L/2)*cos(θ2)
...
# ╔═╡ 93bd2156-f4a5-49a6-9df6-557507bbfaed
function gamma_vector(q, qdot)
	x1, y1, θ1, x2, y2, θ2 = q
	x1d, y1d, θ1d, x2d, y2d, θ2d = qdot
	
	γ = zeros(4)
	
	# First two constraints are linear, so gamma = 0
	γ[1] = 0
	γ[2] = 0
	
	# For revolute joint acceleration terms
	γ[3] = -(L/2)*cos(θ2)*θ2d^2
	γ[4] = -(L/2)*sin(θ2)*θ2d^2
	
	return γ
end

# ╔═╡ 8c58724a-eed5-4c55-8c82-93395c51b690
function applied_forces(q, qdot)
	x1, y1, θ1, x2, y2, θ2 = q
	
	Q = zeros(6)
	
	# Spring force on block in x-direction
	Q[1] = -k*x1
...
# ╔═╡ dc55e2fe-eea7-49b6-afd1-dfa028ac9132
function eom!(du, u, p, t)
	q = u[1:6]
	qdot = u[7:12]
	
	Cq = Cq_matrix(q)
	Q = applied_forces(q, qdot)
	γ = gamma_vector(q, qdot)
...
# ╔═╡ 1680a4d3-b3fe-4160-b5b7-8bfa592add56
begin
	# Initial condition
	θ20 = 35*pi/180      # initial bar angle
	x10 = 0.0
	y10 = 0.0
	θ10 = 0.0
	
	# From revolute joint geometry
	x20 = x10 + (L/2)*cos(θ20)
	y20 = y10 + (L/2)*sin(θ20)
...
# ╔═╡ 30305b35-7d9e-454c-b8eb-de407742a476
begin
	C_error = [norm(constraints(sol.u[i][1:6])) for i in eachindex(sol.u)]
	
	plot(sol.t, C_error,
		xlabel="Time (s)",
		ylabel="Constraint Error",
		title="Constraint Error vs Time",
		legend=false)
end

# ╔═╡ 90652a05-5da0-48fe-b4fb-1ffa9b87f1fd
begin
	anim = @animate for i in 1:5:length(sol.t)
		...
		plot(xlims=(-0.4,0.6), ylims=(-0.2,0.5), aspect_ratio=:equal,
			legend=false, xlabel="x (m)", ylabel="y (m)",
			title="Sliding Block and Compound Pendulum")
		
		# ground
		plot!([-0.5,0.7],[-0.08,-0.08], linewidth=2)
		
		# block
		block_w = 0.12
		block_h = 0.10
		plot!([x1-block_w/2, x1+block_w/2, x1+block_w/2, x1-block_w/2, x1-block_w/2],
		      [y1-block_h/2, y1-block_h/2, y1+block_h/2, y1+block_h/2, y1-block_h/2],
		      linewidth=3)
		
		# pendulum
		plot!([xA,xB],[yA,yB], linewidth=5)
		
		# pin joint
		scatter!([x1],[y1], markersize=6)
	end
	
	gif(anim, "multibody_pendulum.gif", fps=30)
end

# ╔═╡ 9995f242-f7a0-4ac3-a1fb-9ff354185388
function solve_accel_lambda(q, qdot)
	Cq = Cq_matrix(q)
	Q = applied_forces(q, qdot)
	γ = gamma_vector(q, qdot)
...
# ╔═╡ 0a58e91b-5a32-4261-ad4f-e15f68abc959
begin
	lambda_history = zeros(length(sol.t), 4)
	
	for i in eachindex(sol.t)
		q = sol.u[i][1:6]
		qdot = sol.u[i][7:12]
		qddot, λ = solve_accel_lambda(q, qdot)
		lambda_history[i,:] = λ
	end
	
	plot(sol.t, lambda_history,
		xlabel="Time (s)",
		ylabel="Constraint Multipliers",
		title="Lagrange Multipliers vs Time",
		label=["λ1: y1 constraint" "λ2: θ1 constraint" "λ3: joint x" "λ4: joint y"])
end

# ╔═╡ bd258174-afeb-479a-80c2-5c3091653e28
begin
	x1_history = [u[1] for u in sol.u]
	θ2_history = [u[6] for u in sol.u] .* 180/pi
	
	plot(sol.t, x1_history,
		xlabel="Time (s)",
		ylabel="Block Position x1 (m)",
		title="Sliding Block Position",
		legend=false)
end

# ╔═╡ dc0a6e1b-9f2b-4237-a375-98c3aa496311
begin
	plot(sol.t, θ2_history,
		xlabel="Time (s)",
		ylabel="Pendulum Angle θ2 (deg)",
		title="Pendulum Angular Motion",
		legend=false)
end

# ╔═╡ 4fba139f-749a-44ab-8803-94a6e3f107b9
...
git-tree-sha1 = "038d03fdf5091200f4d043c0bcbe41093df3894a"
